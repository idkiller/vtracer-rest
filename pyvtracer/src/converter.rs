...
pub fn convert_image_to_svg_file(config: Config) -> Result<(), String> {
    let config = config.into_converter_config();
    ...
    match svg {
        Ok(svg_file) => write_svg(svg_file, output_path),
        Err(e) => Err(format!("write svg error : {}", e))
    }
}

fn color_image_to_svg(config: ConverterConfig) -> Result<SvgFile, String> {
...
fn binary_image_to_svg(config: ConverterConfig) -> Result<SvgFile, String> {
